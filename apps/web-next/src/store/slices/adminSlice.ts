...
export const adminLogin = createAsyncThunk(
  ...
  async (
    ...
    try {
      ...
      if (isBrowser) localStorage.setItem('adminToken', data.data.token)
...
const adminSlice = createSlice({
  ...
  reducers: {
    ...
    setAdminToken: (state, action: PayloadAction<string>) => {
      ...
      if (isBrowser) localStorage.setItem('adminToken', action.payload)
    },
  },
...
// Export selectors
export const selectAdminUser = (state: { admin: AdminState }) =>
  state.admin.user
export const selectAdminToken = (state: { admin: AdminState }) =>
  state.admin.token
