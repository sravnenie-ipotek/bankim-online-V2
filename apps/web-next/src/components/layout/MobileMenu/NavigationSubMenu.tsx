    ...
    >
      <div className="p-6 pt-20">
        <button
          type="button"
          onClick={onClose}
          className="mb-6 text-accent-primary bg-transparent border-none cursor-pointer text-[16px] flex items-center gap-2"
        >
        ...
        <ul className="flex flex-col gap-4 list-none p-0 m-0">
          {items.map((item) => (
            <li
              key={item.title}
              className="text-[18px] text-white hover:text-accent-primary transition-colors"
            >
              <Link
                href={item.path || '/'}
                onClick={handleLinkClick}
                className="text-inherit no-underline block py-2"
              >
                {item.title}
              </Link>
            </li>
