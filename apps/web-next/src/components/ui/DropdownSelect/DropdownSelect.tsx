...
import type { DropdownSelectProps } from './interfaces/DropdownSelectProps';
...
const DropdownSelect: React.FC<DropdownSelectProps> = ({
  ...
  return (
    <div ref={wrapperRef} className="flex flex-col gap-1 w-full relative">
      ...
      >
        <span
          className={`text-[1.125rem] font-normal truncate ${isPlaceholder ? 'text-[#848484]' : 'text-white'}`}
        >
          {displayLabel}
        </span>
        {isOpen ? (
      ...
      {isOpen ? (
        <div className="absolute top-full left-0 right-0 mt-1 z-dropdown rounded border border-base-secondaryDefaultButton bg-base-secondary shadow-[0px_8px_32px_0px_rgba(0,0,0,0.16)] max-h-[300px] overflow-auto">
          {searchable ? (
