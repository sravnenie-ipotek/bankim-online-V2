...
const ChangeLanguage: React.FC = () => {
  ...
  return (
    <div ref={wrapperRef} className="shrink-0 relative" style={{ width: 230, height: 54 }}>
      {/* Trigger button */}
      <div
        className="rounded-md flex items-center justify-between px-4 border border-base-secondaryDefaultButton bg-base-primary cursor-pointer w-full h-full box-border"
      ...
      >
        <div className="bg-transparent flex gap-2 items-center justify-center flex-1 min-w-0 text-center">
      ...
      {isOpen && (
        <div
          className="absolute mt-2.5 z-[9999] rounded-md py-2 border border-base-secondaryDefaultButton bg-base-secondary shadow-[0px_8px_32px_0px_rgba(0,0,0,0.16)] overflow-auto"
          style={{ width: 520, height: 540 }}
        >
          <div className="text-3xs not-italic font-semibold leading-normal text-white text-start py-2 px-4">
          ...
          {LANGUAGE_OPTIONS.map((item) => (
            <div
              key={item.value}
              tabIndex={0}
              className="flex items-center cursor-pointer justify-between py-2 px-4 hover:bg-base-secondaryHoveredButton transition-colors"
