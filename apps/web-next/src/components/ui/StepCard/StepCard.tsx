    ...
    return (
      <div className={rootClassName}>
        <div className="relative box-border h-full py-[clamp(12px,10%,24px)] px-[clamp(12px,6%,24px)] xs:w-full sm:px-0 lg:py-0 lg:px-0">
            ...
            {/* Inner content box: lg 316×150 (@1440); xl 421×200 (@1920). */}
            <div className="w-[316px] max-w-full min-w-0 h-[135px] min-h-[135px] sm:h-[150px] sm:min-h-0 lg:w-[clamp(260px,28.7vw,316px)] lg:h-[clamp(120px,28.25vh,150px)] lg:min-h-0 xl:w-[421px] xl:h-[200px] xl:min-h-0 lg:py-[clamp(6px,0.69vh,10px)] xl:py-[clamp(8px,0.92vh,13px)] lg:px-0 lg:box-border lg:relative lg:overflow-visible mx-auto lg:mx-auto">
              {/* Step number column: lg 90×150 (@1440); xl 120×200 (@1920). */}
              <span
                aria-hidden
                className="absolute z-0 pointer-events-none select-none flex items-center justify-end rtl:justify-start top-1/2 -translate-y-1/2 right-0 left-auto rtl:right-auto rtl:left-0 w-[90px] h-[135px] sm:h-[150px] lg:w-[90px] lg:h-[150px] xl:w-[120px] xl:h-[200px] lg:right-0 lg:left-auto rtl:lg:left-0 rtl:lg:right-auto [container-type:size]"
              >
                {/* Step number: Inter via font-inter (Tailwind theme.fontFamily.inter, --font-inter from layout) */}
                <span className="font-inter font-medium tabular-nums text-[rgba(51,53,53,0.55)] leading-[0.7] text-[100cqh]">
                  {stepNumber}
                </span>
              </span>
              {/* Text column: full width so text can overlap the number (like tablet); number stays z-0, text z-10. */}
              <div className="relative z-10 w-full max-w-full h-full min-w-0 font-he text-left text-white rtl:text-right flex flex-col justify-center items-start rtl:items-end">
                <Image
