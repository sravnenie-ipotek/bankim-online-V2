...
const CalculateMortgage: React.FC = () => {
  ...
  const stepLocation =
    !isNaN(step) && step >= 1 && step <= TOTAL_STEPS
      ? `mortgage_step${step}`
      : 'mortgage_calculation';
  useContentFetch('mortgage_calculation', stepLocation);
  ...
  return (
    <>
      {stepNumber !== '4' && <ProgressBar progress={stepNumber} data={progressLabels} />}
      {renderStep()}
    </>
  );
};
