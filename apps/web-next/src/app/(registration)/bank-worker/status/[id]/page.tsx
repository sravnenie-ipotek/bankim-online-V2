...
/**
 * Bank worker status page: fetches status via RTK (fetchBankWorkerStatus), displays timeline and link to personal cabinet.
 */
const BankWorkerStatus: React.FC = () => {
        ...
        {statusData ? (
          <div className="flex flex-col gap-6">
            ...
            <div className="flex flex-col gap-2 text-sm">
              ...
              <div className="flex justify-between">
                <span className="text-textTheme-secondary">{getContent('position')}</span>
                <span className="text-textTheme-primary">{statusData.position}</span>
              </div>
            </div>

            {statusData.timeline && statusData.timeline.length > 0 && (
              <div className="flex flex-col gap-3">
                <h3 className="text-sm font-medium text-textTheme-primary">
                  {getContent('timeline')}
                </h3>
                {statusData.timeline.map((item, idx) => (
                  <div key={idx} className="flex gap-3 items-start">
                    ...
                    <div>
                      <span className="text-xs text-textTheme-secondary">{item.date}</span>
                      <p className="text-sm text-textTheme-primary">{item.event}</p>
                    </div>
                  </div>
                ))}
              </div>
