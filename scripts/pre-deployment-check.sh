...
check_error "Frontend has version:update script" \
    "grep -q '\"version:update\":' apps/web/package.json" \
    "Add version:update script to apps/web/package.json"
...
check_error "apps/api/dist/main.js exists" \
    "[ -f apps/api/dist/main.js ]" \
    "apps/api/dist/main.js is missing - run 'turbo build' first"
...
check_warning "Frontend build directory configured" \
    "[ -d apps/web/build ] || [ -d apps/web/dist ]" \
...
if ! grep -q "\"version:update\":" apps/web/package.json 2>/dev/null; then
    echo "# Add to apps/web/package.json scripts:"
